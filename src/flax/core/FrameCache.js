    ...
    _createSpriteFrames: function(file, dict, texture) {
        ...
        for (var key in frames) {
            ...
            if(offset.x != 0 || offset.y != 0){
                trim = flax.rect(offset.x, offset.y, rect.width, rect.height);
            }

            var orig = flax.rect(rect.x, rect.y, sourceSize.x, sourceSize.y);
            var t = new PIXI.Texture(texture.baseTexture, rect, orig, trim, rotated);
