...
flax.GunParam = flax.Class.extend({
...
flax.Gun = cc.Node.extend({
    ...
    shootOnce:function()
    {
        ...
        if(this.param.fireSound && flax.playSound) flax.playSound(this.param.fireSound);
    },
...
flax.BulletCanvas = flax.SpriteBatchNode.extend({
    assetsFile:null,
    onBulletHit:null,
    onBulletOut:null,
    _bullets:null,
...
//                continue;
            ...
            rect = flax.getBounds(b, true);
            hitted = false;
            targets = null;
            var outOfBounds = !flax.rectIntersectsRect(flax.stageRect, rect);
            if(!outOfBounds){
                ...
                if(targets && targets.length){
                    ...
                    if(radius > 0){
                        rect = flax.rect(pos.x - radius/2, pos.y - radius/2, radius, radius);
...
flax.BulletCanvas.fetch = function (assetsFile) {
    ...
    var texturePath = flax.path.changeBasename(assetsFile, ".png");
